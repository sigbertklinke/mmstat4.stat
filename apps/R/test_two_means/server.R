...
shinyServer(function(input, output, session) {
  ...
  output$outputTestPlot <- renderPlot({
    ...
    xlim  <- qnorm(c(0.001, 0.999), mean=0, sd=sqrt(s2pop1/30+s2pop2/30))
    x     <- xlim[1]+diff(xlim)*(0:300)/300
    y     <- dnorm(x, mean=0, sd=sgpop)
    my    <-max(y)
    ylim  <- my*c(-0.5, 1)
    par (mar=c(5,0,2,0))
    plot (x, y, type='l', xlab="", xlim=xlim, ylim=ylim, axes=F,
          main=mmstat.math("&H[0];: &mu[1]==mu[2]; vs. &H[1];: &mu[1]!=mu[2]; "), ylab="",
    ...
    dist <- qt(1-oma/2, n1+n2-2)*sqrt(((n1-1)*s12+(n2-1)*s22)/(n1+n2-2)*(1/n1+1/n2)) 
    mmstat.plotTestRegions(c(-dist, dist), 
                           xlim=usr[1:2], ylim=-my*c(0.05, 0.25), cex=inp$cex, col=mmstat$col[[9]],
                           label=mmstat.math(gettext(" &sigma[1]==sigma[2]; ")), pos=4)
    # unequal variances    
    df   <- floor((s12/n1+s22/n2)^2/(s12^2/n1^2/(n1-1)+s22^2/n2^2/(n2-1)))
    dist <- qt(1-oma/2, df)*sqrt(s12/n1+s22/n2) 
    mmstat.plotTestRegions(c(-dist, dist), 
                           xlim=usr[1:2], ylim=-my*c(0.30, 0.50), cex=inp$cex,  col=mmstat$col[[10]],
                           label=mmstat.math(gettext(" &sigma[1]!=sigma[2]; ")), pos=4)
    #
    mmstat.axis(1, xlim, cex.axis=inp$cex)
    abline(v=samples[[1]]$mean-samples[[2]]$mean, col=mmstat$col[[2]], lwd=2, lty="dotted")
    abline(v=mupop1-mupop2, col=mmstat$col[[1]], lwd=2, lty="dotted")
    abline(v=0, col="gray", lwd=2)
    
    xp <- qnorm(0.75, 0, sgpop)
    yp <- dnorm(xp, 0, sgpop)
    text(xp, yp, mmstat.math(" &bar(X)[1]-bar(X)[2]%~~%N(0*','*sigma[1]^2/n[1]+sigma[2]^2/n[2]); "), pos=4, cex=inp$cex)
    box() 
  })
  ...
  output$outputSamplePlot <- renderPlot({
    ...
    text(usr[2], 0.1, paste(gettext(lgrp[1]), '(1)'), pos=2, cex=0.75*inp$cex)
    grp2 <-  grp$values==lgrp[2]
    drawIqrBoxWithPoints(var$values[grp2], var$jitter[grp2], ylim=0.25+c(0.0, 0.2),
                         box.param=list(border=mmstat$col[[1]], lwd=2),
                         points.param=list(col=mmstat$col[[9]], pch=19, cex=0.5*inp$cex))
    text(usr[2], 0.35, paste(gettext(lgrp[2]), '(2)'), pos=2, cex=0.75*inp$cex)
