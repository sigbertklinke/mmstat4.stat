library("shiny")
source("helper2.R")
mmstat.ui.elem('trend', 'radioButtons',
               label    = gettext("Choose trend type"), 
               choices  = gettext(c('NO.TREND', 'LIN.TREND', 'EXP.TREND', 'MOV.AVE.3',  'MOV.AVE.5', 'MOV.AVE.7'), "name"),
               selected = 'NO.TREND')
mmstat.ui.elem('season', 'radioButtons',
               label    = gettext("Choose season type"), 
               choices  = gettext(c('NO.SEASON', 'ADD.SEASON', 'MUL.SEASON'), "name"),
               selected = 'NO.SEASON')
mmstat.ui.elem('quality', 'checkboxInput',
               label   = gettext("Show model quality"), 
               value   = FALSE)
mmstat.ui.elem('dataset', 'dataSet',
               label   = gettext("Select set of  time series set"),
               choices = mmstat.getDataNames("INDEX-BASIC-RENT-BERLIN", "TELEPHONE"))
mmstat.ui.elem("variable",   "variable1", 
               label   = gettext("Select times series"),
               vartype="numeric")
mmstat.ui.elem("cex",        "fontSize")

add.r2 <- function (trendmodel="NO.TREND", seasonality="NO.SEASON", value=NA) {
...
decompose.ts <- function (ts, trendmodel="NO.TREND", seasonality="NO.SEASON") {
  ...
  if (trendmodel=="LIN.TREND") {
    trend <- fitted(lm(ts~t))
  }
  if (trendmodel=="EXP.TREND") {
    xt    <- log(ts)
    trend <- exp(fitted(lm(xt~t)))
  }  
  if (trendmodel=='MOV.AVE.3') {
    trend <- filter(ts, rep(1/3,3))
  }
  if (trendmodel=='MOV.AVE.5') {
    trend <- filter(ts, rep(1/5,5))
  }
  if (trendmodel=='MOV.AVE.7') {
    trend <- filter(ts, rep(1/7,7))
  }
  random   <- ts-trend
  seasonal <- rep(NA, length(ts))
  figure   <- rep(NA, length(ts))
  season   <- 1+(t%%frequency(ts))
  ...
  ret <- list(ts=ts, 
              trend=ts(trend, start=start(ts), frequency=frequency(ts)),
              seasonal=ts(seasonal, start=start(ts), frequency=frequency(ts)),
              random=ts(random, start=start(ts), frequency=frequency(ts)),
              figure=ts(figure, start=start(ts), frequency=frequency(ts)), 
              trendmodel=trendmodel, seasonality=seasonality,
              r2=1-var(random, na.rm=T)/var(ts))
...
plot.tsd <- function (fit, cex) {
  ...
  if (fit$seasonality!='NO.SEASON') {
    ...
    par(mai=c(0.7,oldpar$mai[2], 0, oldpar$mai[4]))
    bcex <- 1.5
  }
  if (fit$trendmodel=='NO.TREND') {
    plot(fit$ts, axes=F, xlab="")
    axis(1, cex.axis=bcex*cex)
    mmstat.axis(2, ylim, cex.axis=bcex*cex)
    title(xlab=gettext("Time"), cex.lab=bcex*cex)
    box()
  } else {
    ts.plot(fit$ts, fit$trend, gpars=list(col=c("black", "red"), axes=F, xlab=""))
    axis(1, cex.axis=bcex*cex)
    mmstat.axis(2, ylim, cex.axis=bcex*cex)
    title(xlab=gettext("Time"), cex.lab=bcex*cex)
    box()
  ...
  if (fit$seasonality!='NO.SEASON') {
    par(mai=c(0,oldpar$mai[2], 0, oldpar$mai[4]))
    #plot(fit$trend, ylim=range(fit$trend, na.rm=T), ylab=gettext("trend"), xaxt = "n")
    ylim <- range(fit$seasonal, na.rm=T)
    plot(fit$seasonal, ylim=ylim, axes=F, ylab="")
    mmstat.axis(2, ylim, cex.axis=bcex*cex)
    title(ylab=gettext("seasonal"), cex.lab=bcex*cex)
    box()
    #par(mai=c(1,oldpar$mai[2], 0, oldpar$mai[4]))
    ylim <- range(fit$random, na.rm=T)
    plot(fit$random, ylim=ylim, ylab="", axes=F)
    mmstat.axis(2, ylim, cex.axis=bcex*cex)
    title(ylab=gettext("random"), cex.lab=bcex*cex)
    box()
  }
}
...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    mmstat.ui.update('variable', session=session, choices=gettext(mmstat$dataset[[inp$dataset]]$numvars, "names")) 
  })
  
  output$seasonUI <- renderUI({
    var  <- getVar()
    if (frequency(var$values)==1) return (p(gettext("Frequency of time series is one, therefore no seasonality estimation is possible!")))
  ...
  output$outputPlot <- renderPlot({
    ...
    inp  <- mmstat.getValues(NULL, 
                             ...
                             cex=input$cex)
    if ((frequency(var$values)==1)||(inp$trend=='NO.TREND')) mmstat.ui.update('season', session=session)
