...
mmstat.ui.elem("varequal",   "checkboxInput", label=gettext("Equal variances assumed"), value=FALSE)
...
mmstat.ui.elem("dataset",    "dataSet",     choices=mmstat.getDataNames("ALLBUS2012-GENERAL", "ALLBUS2004-GENERAL", "ALLBUS2002-GENERAL"))
...
CItwomeans  <-function (x1, x2, oma, var.equal=F, xlim=NULL) {
  ...
  s22 <- var(x2)
  if (var.equal) {
    dist <- qt(1-(1-oma)/2, n1+n2-2)*sqrt(((n1-1)*s12+(n2-1)*s22)/(n1+n2-2)*(1/n1+1/n2)) 
  } else {
  ...
  list(upper=upper, lower=lower, mean=mean(x1)-mean(x2), oma=oma, n1=n1, n2=n2, xlim=lim)
}
...
shinyServer(function(input, output, session) {  
  ...
  output$varequalUI  <- renderUI({ mmstat.ui.call('varequal') })
  ...
  drawSample <- reactive ({
    inp    <- mmstat.getValues(NULL, go=input$go, size1=input$size1, size2=input$size2, 
                               conflevel=input$conflevel, varequal=input$varequal,
                               speed=input$speed)
  ...
  output$outputConfPlot <- renderPlot({
    ...
    if (nci) {
      ...
      text(meandiff, 1.5, sprintf("%.*f", var$decimal, meandiff), col=mmstat$col[[1]], pos=4, cex=inp$cex)
      usr <- par("usr")
 ...
 #   points(sample, 1.5+0.5*jitter[index], pch=19, col=mmstat$col[[1]])
      ...
      for (i in seqi) {
        ...
        col <- ifelse((rng[1]>meandiff) || (rng[2]<meandiff), 'black', mmstat$col[[2]])
...
# str(index)
      ...
      text(posx[1], 1.95+0.2*index, labels=sprintf('%.0f;%.0f', n1[index], n2[index]), cex=inp$cex)
      oma  <- sapply(confint, '[[', 'oma')
      index <- 1+c(0, which(diff(oma)!=0))
      text(posx[2], 1.95+0.2*index, labels=sprintf('%.3f', oma[index]), cex=inp$cex)
      axis(3, at=posx, labels=c(expression('n1;n2'), expression(alpha)), cex.axis=inp$cex)
  ...
  output$outputSamplePlot <- renderPlot({
    ...
    drawIqrBoxWithPoints(var$values[grp1], var$jitter[grp1], ylim=c(0, 0.2),
                         box.param=list(border=mmstat$col[[1]], lwd=2),
                         points.param=list(col=mmstat$col[[9]], pch=19, cex=0.5*inp$cex))
    text(usr[2], 0.1, paste(gettext(samples[[1]]$group), '(1)'), pos=2, cex=0.75*inp$cex)
    grp2 <-  grp$values==lgrp[2]
    drawIqrBoxWithPoints(var$values[grp2], var$jitter[grp2], ylim=0.25+c(0.0, 0.2),
                         box.param=list(border=mmstat$col[[1]], lwd=2),
                         points.param=list(col=mmstat$col[[9]], pch=19, cex=0.5*inp$cex))
    text(usr[2], 0.35, paste(gettext(samples[[2]]$group), '(2)'), pos=2, cex=0.75*inp$cex)
    drawIqrBoxWithPoints(samples[[1]]$values, samples[[1]]$jitter, ylim=0.5+c(0, 0.2*sqrt(length(samples[[1]]$values)/grp$tab[1])),
                         box.param=list(border=mmstat$col[[2]], lwd=2),
                         points.param=list(col=mmstat$col[[10]], pch=19, cex=0.5*inp$cex))
    drawIqrBoxWithPoints(samples[[2]]$values, samples[[2]]$jitter, ylim=0.75+c(0, 0.2*sqrt(length(samples[[2]]$values)/grp$tab[2])),
                         box.param=list(border=mmstat$col[[2]], lwd=2),
                         points.param=list(col=mmstat$col[[10]], pch=19, cex=0.5*inp$cex))
    box()
  })
