...
shinyServer(function(input, output, session) {
  ...
  output$outputTestPlot <- renderPlot({
    ...
    if (is.list(sample)) {
      ...
      xlim  <- range(qnorm(c(0.00001, 0.99999),  inp$mu0, var$sd/sqrt(30)),
                     qnorm(c(0.00001, 0.99999), var$mean, var$sd/sqrt(30)))
      ...
      plot (x, y, type='l', xlab="", xlim=xlim, ylim=ylim, axes=F,
            main=mmstat.math("&H[0];: &mu==mu[0]; vs. &H[1];: &mu!=mu[0]; "), ylab="",
      ...
      text(xp, yp, mmstat.math(" &bar(X);~&N(mu[0], sigma^2/n); "), pos=4, cex=inp$cex)
      ...
      mmstat.plotTestRegions(inp$mu0+sample$sd/sqrt(sample$n)*c(-c, c), 
                             ...
                             label=mmstat.math(gettext(" &sigma; estimated")), pos=2+2*(var$mean>inp$mu0))
      ...
      text(sample$mean, ypos, sprintf('%.*f',  var$decimal, sample$mean), 
           pos=4-2*(sample$mean<var$mean), col=mmstat$col[[2]], cex=inp$cex)
      text(var$mean, ypos, sprintf('%.*f', var$decimal, var$mean), 
           pos=2+2*(sample$mean<var$mean), col=mmstat$col[[1]], cex=inp$cex)
      box() 
    }
  })
  ...
  output$outputSamplePlot <- renderPlot({
    ...
    drawIqrBoxWithPoints(samp$values, samp$jitter, ylim=0.5+c(0, 0.45*sqrt(length(samp$values)/var$n)),
