...
mmstat.ui.elem("dataset",    "dataSet",     choices=mmstat.getDataNames("CREDIT", "BOSTONHOUSING"))
...
drawVariableValues <- function (x, jitter=NULL, ylim, box.param=NULL, points.param=NULL, param=NULL) {
  if (is.numeric(x)) {
...
shinyServer(function(input, output, session) {
  ...
  doTest <- reactive({
    var    <- getVar()
    sample <- drawSample()
    inp    <- mmstat.getValues(NULL, pi0=input$pi0, test=input$test, alpha=input$alpha)
    test   <- list(pi0=inp$pi0, statistic=sample$prop[1], cond=sample$n*inp$pi0*(1-inp$pi0),
                   var=inp$pi0*(1-inp$pi0)/sample$n, alpha=inp$alpha,
    ...
    if (inp$test=='two.sided') {
      ...
      test$cu         <- inp$pi0-qnorm(1-mmstat$alpha[inp$alpha]/200)*sqrt(inp$pi0*(1-inp$pi0)/sample$n)
      test$co         <- inp$pi0+qnorm(1-mmstat$alpha[inp$alpha]/200)*sqrt(inp$pi0*(1-inp$pi0)/sample$n)
    }
    if (inp$test=='less') {
      test$h0.html    <- '&pi;&ge;&pi;<sub>0</sub>'
      test$h1.html    <- '&pi;&lt;&pi;<sub>0</sub>'
      test$hypotheses <- mmstat.math("&H[0];: &pi>=pi[0]; vs. &H[1];: &pi<pi[0]; ")
      test$cu         <- inp$pi0-qnorm(1-mmstat$alpha[inp$alpha]/100)*sqrt(inp$pi0*(1-inp$pi0)/sample$n)
      test$co         <- +Inf
    }
    if (inp$test=='greater') {
      ...
      test$hypotheses <- mmstat.math("&H[0];: &pi<=pi[0]; vs. &H[1];: &pi>pi[0]; ")
      test$cu         <- -Inf
      test$co         <- inp$pi0+qnorm(1-mmstat$alpha[inp$alpha]/100)*sqrt(inp$pi0*(1-inp$pi0)/sample$n)
    }
    test$decision     <- ifelse((sample$prop[1]<test$cu)||(sample$prop[1]>test$co), gettext("Reject H<sub>0</sub>"),  gettext("Can not reject H<sub>0</sub>"))
    test$cond.text    <- ifelse(sample$n*inp$pi0*(1-inp$pi0)<9, mmstat.math(gettext("Normal approximation of test statistics &hat(Pi); is NOT applicable!")), "")
    test
  })
  ...
  output$outputPlot <- renderPlot({
    ...
    sample <- drawSample()
    test   <- doTest()
    inp    <- mmstat.getValues(NULL, alpha = input$alpha, pi0 = input$pi0, cex = input$cex, test = input$test)
    xi <- (0:100)/100
    ...
    xlim <- c(0, 1)
    par (mar=c(5,0,2,0))
    plot(xi, yi, type="l", xlab=mmstat.math(" &pi; "), ylab="", ylim=ylim, xlim=xlim, axes="F",
         cex.axis=inp$cex, cex.lab=inp$cex, cex.main=1.2*inp$cex, cex.sub=inp$cex)
    title(main=test$hypotheses, sub=test$cond.text, col.sub="red", cex=inp$cex)
    mmstat.axis(1, c(0,1))
...
#    }
    ...
    mmstat.plotTestRegions(c(test$cu, test$co), xlim=c(0,1), ylim=-ya*c(0.05, 0.4), cex=inp$cex, close=T)
    drawVariableValues(as.factor(var$values), ylim=-ya*c(0.75, 1), 
                       param=list(cex=inp$cex, col=0, border=NA, level=T))
    drawVariableValues(as.factor(sample$values), ylim=-ya*c(0.45, 0.45+0.25*sqrt(sample$n/var$n)), 
