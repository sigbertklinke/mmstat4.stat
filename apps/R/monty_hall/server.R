...
pdc <- gettext(c("to left door", "to middle door", "to right door", "randomly to a door"), "num")
aoc <- gettext(c('keep door', 'change door'), "num")

mmstat.ui.elem("pointdoor", 'radioButtons', selected=1,
               label    = gettext("1. Guest points"), choices  = pdc)
mmstat.ui.elem("afteropen", 'radioButtons', selected=1,
               label    = gettext("3. Guest decides to"), choices  = aoc)
...
points <- function(deal) {
  out <- rep('Empty.png', 3)
  if (!is.na(deal$guest)) out[deal$guest] <- 'PointTo.png'
  out
}

doors <- function(deal) {
  out <- rep('GoatClose.png', 3)
  if (!is.na(deal$price)) out[deal$price] <- 'CarClose.png'
  if (!is.na(deal$host))  out[deal$host] <- 'GoatOpen.png'
  if (!is.na(deal$final)) {
    out[deal$final] <- if (deal$final==deal$price) 'CarOpen.png' else 'GoatOpen.png'
  }
  out
}
...
shinyServer(function(input, output, session) {
  ...
  output$hostopensUI <- renderText({ gettext("2. Host opens door with a goat") })
  ...
  observeEvent(input$host, {
    isolate({
      ...
      deal$host <- if (length(door)==1) door else sample(door, 1)
      if (input$afteropen==1) deal$final <- deal$guest
      if (input$afteropen==2) deal$final <- setdiff(1:ndoor, c(deal$host, deal$guest))
  ...
  observeEvent(input$guest, {
    isolate({
      ...
      if (inp$pointdoor==1) deal$guest <- 1
      if (inp$pointdoor==2) deal$guest <- 2
      if (inp$pointdoor==3) deal$guest <- 3
      if (inp$pointdoor==4) deal$guest <- sample(3,1)
      deal$host  <- NA
      deal$final <- NA
    })
  })
  ...
  observe({
    inp    <- mmstat.getValues(NULL, speed=input$speed,  pointdoor=input$pointdoor, afteropen=input$afteropen)
    if (inp$speed>0) {
      # guest
      isolate({
        deal$trial <- deal$trial+1  
  ...
  output$distPlot <- renderPlot({
    ...
    plot(cumsum(deal$success)/seq(length(deal$success)), xlim=c(1, deal$trial), ylim=c(0,1), pch=19, 
         xlab=gettext("Trial"), ylab=gettext("Relative frequency of successful trials"),
         main=main,
         cex.axis=input$cex,
         cex.lab=input$cex,
         cex.main=1.2*input$cex,
         cex.sub=input$cex)
    abline(h=(1:2)/3, col="gray20")
  })
  
  output$imagePlot <-renderText({
    d <- doors(deal)
    p <- points(deal)
    paste0('<table width="100%"><tr align="center"><td width="10%"><h1>',
           deal$trial,
