...
mmstat.ui.elem("graph", 'radioButtons',
               ...
               choices  = gettext(c("2D.SCATTERPLOT", "3D.SCATTERPLOT", "SCATTERPLOTMATRIX"), "name"),
...
mmstat.ui.elem("coeff", 'checkboxGroupInput',
               ...
               choices  = gettext(c("SHOW.BRAVAIS.PEARSON", "SHOW.SPEARMAN"), "name"),
               value    = character())
mmstat.ui.elem("dataset",    "dataSet",     
               choices  = mmstat.getDataNames("USCRIME", "CARS", "DECATHLON"))
mmstat.ui.elem('variableSelect',  'variableN',  vartype="numeric",
...
#  })
  ...
  output$scatterPlot <- renderPlot({
    ...
    clist <- cycleList()
    if (nrow(clist)==0) {
      plot(0,0, xlim=c(-0.1, 0.1), ylim=c(-0.1, 0.1), type="n", axes=F, xlab="", ylab="", pch=19, cex=0.5)
      text(0,0, gettext("Please select two, three or more variables!"), cex=1.5*inp$cex)
    } else {
      ...
      ct <<- ct%%nrow(clist)
      varx <- mmstat.getVar(isolate(inp$dataset), varname=clist[ct+1,1], na.action=na.pass)
      if (varx$name==clist[ct+1,1]) {
        if (inp$graph=='2D.SCATTERPLOT') {
          vary <- mmstat.getVar(inp$dataset, varname=clist[ct+1,2], na.action=na.pass)
          plot (varx$values, vary$values, xlab=gettext(clist[ct+1,1]), ylab=gettext(clist[ct+1,2]),
          ...
          if (length(inp$coeff)) {
            ...
            for (i in seq(inp$coeff)) {
              if (inp$coeff[i]=="SHOW.BRAVAIS.PEARSON") {
                main[i] <- sprintf(" &r[xy];=%+.2f", cor(varx$values, vary$values, use="c"))
              }
              if (inp$coeff[i]=="SHOW.SPEARMAN") {        
                main[i] <- sprintf(" &r[s];=%+.2f", cor(varx$values, vary$values, use="c", method="s"))
              }
            } 
        ...
        if (inp$graph=='3D.SCATTERPLOT') {
          vary <- mmstat.getVar(isolate(inp$dataset), varname=clist[ct+1,2], na.action=na.pass)
          varz <- mmstat.getVar(isolate(inp$dataset), varname=clist[ct+1,3], na.action=na.pass)
          scatterplot3d (varx$values, vary$values, varz$values, pch=19, cex=0.75,
                         cex.axis=0.8*inp$cex, cex.lab=inp$cex, cex.main=inp$cex,
                         highlight.3d=T,
                         xlab=gettext(clist[ct+1,1]), 
                         ylab=gettext(clist[ct+1,2]), 
                         zlab=gettext(clist[ct+1,3]))
          if (length(inp$coeff)) title(main=gettext("Coefficients are not available for 3D Scatterplot"))
        }
        if (inp$graph=='SCATTERPLOTMATRIX') {
          dat <- data.frame(varx$values)
          for (i in 2:length(clist[ct+1,])) {
            var     <- mmstat.getVar(inp$dataset, varname=clist[ct+1,i], na.action=na.pass)
            dat[,i] <- var$values
          }
          colnames(dat) <- gettext(clist[ct+1,])
#          pairs(as.matrix(dat), labels=gettext(clist[ct+1,]), 
#                cex.labels=inp$cex, 
#                upper.panel=upper.panel, coeff=inp$coeff)
          splom(~dat,  pch=19, col="black", as.matrix=T,
                cex=1/sqrt(ncol(dat)), varname.cex=1.5*inp$cex/inp$cex/(ncol(dat)^(1/3)),
                axis.text.cex=ifelse(ncol(dat)<5, inp$cex/(ncol(dat)^(1/3)), 0),
                coeff=inp$coeff, cex.coeff=1.5*inp$cex/(ncol(dat)^(1/3)),
                  ...
                  if (length(coeff)) {
                    if(i>j) {
                      ...
                      for (i in seq(inp$coeff)) {
                        ...
                        if (inp$coeff[i]=="SHOW.SPEARMAN") {        
                          main[i] <- sprintf(" &r[s];=%+.2f", cor(x, y, use="c", method="s"))
                        }
                      }
                      xpos <- mean(range(x))
                      ypos <- min(y)+diff(range(y))*c(1,3,5)/(2*length(coeff))
                      for (i in seq(main)) panel.text(xpos, ypos[i], mmstat.math(main[i]), cex=cex.coeff)
                    }
                  } else {
                    panel.points(x,y, cex=cex, pch=19, col="black")
                  }
